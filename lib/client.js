...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 对外发布：把 wasm/ 下 4 个文件提交到 GitHub 仓库，用 raw.githubusercontent.com 直链
    //   （raw 支持单文件 100MB 且带 CORS 头；26MB 的 .data 能正常跨域加载）。
    //   本地开发：由 voice_server.py 提供静态服务，用第二个地址兜底。
    // 注意：jsDelivr 单文件限 20MB，26MB 的 .data 放不下，所以不用 jsDelivr。
    const WASM_BASES = [
      "https://raw.githubusercontent.com/widesignal0208/dsh-voice-input/main/wasm/",
      "http://127.0.0.1:8123/wasm/",
    ];
    ...
    function loadWasmRecognizer() {
      ...
      wasmPromise = new Promise((resolve, reject) => {
        const M = {};
        window.Module = M;
        M.locateFile = (path) => base + path; // .wasm / .data 都走静态服务
        M.setStatus = () => {};
        ...
        loadScript(base + "sherpa-onnx-asr.js")
          .then(() => loadScript(base + "sherpa-onnx-wasm-main-asr.js"))
          .catch(reject)
          .finally(() => clearTimeout(timeout));
      }).catch((e) => {
        // 当前地址失败 → 尝试下一个地址；全部失败才标记 wasm 不可用
        wasmPromise = null;
        wasmBaseIndex += 1;
        if (wasmBaseIndex < WASM_BASES.length) return loadWasmRecognizer();
        wasmFailed = true;
        throw e;
      });
      return wasmPromise;
